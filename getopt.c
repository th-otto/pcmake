...
static void print_to_stderr(const char *format, ...)
{
	va_list args;
...
static void print_to_devnull(const char *format, ...)
{
	(void) format;
}
				   ...
				   int long_only, struct _getopt_data *d, int posixly_correct)
{
	...
	{
		d->opterr = 0;
		d->print_error = print_to_devnull;
	}
...
# define NONOPTION_P ((argv[d->optind][0] != '-' && argv[d->optind][0] != '+') || argv[d->optind][1] == '\0')
		...
		&& (argv[d->optind][1] == '-' || (long_only && (argv[d->optind][2] || !strchr(optstring, argv[d->optind][1])))))
	{
		...
		if (ambig && !exact)
		{
			d->print_error(_("%s: option '%s' is ambiguous\n"), d->argv0, argv[d->optind]);
		...
		if (pfound != NULL)
		{
			...
			if (*nameend)
			{
				...
				} else
				{
					...
					{
						/* --option */
						d->print_error(_("%s: option '--%s' doesn't allow an argument\n"), d->argv0, pfound->name);
					} else
					{
						/* +option or -option */
						d->print_error(_("%s: option '%c%s' doesn't allow an argument\n"),
			...
			} else if (pfound->has_arg == required_argument)
			{
				...
				{
					d->print_error(_("%s: option '--%s' requires an argument\n"), d->argv0, pfound->name);
					d->__nextchar += strlen(d->__nextchar);
					d->optopt = pfound->val;
					return optstring[0] == ':' ? ':' : '?';
				}
		...
		if (!long_only || argv[d->optind][1] == '-' || strchr(optstring, *d->__nextchar) == NULL)
		{
			...
			{
				/* --option */
				d->print_error(_("%s: unrecognized option '--%s'\n"), d->argv0, d->__nextchar);
			} else
			{
				/* +option or -option */
				d->print_error(_("%s: unrecognized option '%c%s'\n"), d->argv0, argv[d->optind][0], d->__nextchar);
			}
...

	{
		...
		if (temp == NULL || c == ':' || c == ';')
		{
			if (d->__posixly_correct)
				...
				d->print_error(_("%s: illegal option -- %c\n"), d->argv0, c);
			else
				d->print_error(_("%s: invalid option -- '%c'\n"), d->argv0, c);
			d->optopt = c;
			return '?';
		}
		...
		if (temp[1] == ':')
		{
			...
			} else
			{
				...
				{
					/* 1003.2 specifies the format of this message.  */
					d->print_error(_("%s: option requires an argument -- '%c'\n"), d->argv0, c);
					d->optopt = c;
					if (optstring[0] == ':')
						c = ':';
					else
				 ...
				 const struct option *longopts, int *longind, int long_only, int posixly_correct)
{
	...
	getopt_data.print_error = getopt_data.opterr ? print_to_stderr : print_to_devnull;
